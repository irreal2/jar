    ...
    public String homeContent(boolean filter) {
        try {
            ...
            if (!rule.optJSONObject("筛选").isEmpty()) {
                filterName = "筛选";
            } else if  (!rule.optJSONObject("filter").isEmpty()) {
                filterName = "filter";
            } else if  (!rule.optJSONObject("filter").isEmpty()) {
                filterName = "filterdata";
            }
            filterJson = rule.optJSONObject(filterName);
...
//获取分类页网址
    ...
    public String detailContent(List<String> ids) {
        try {
           ...
           if ((getRuleVal("xlbiaotiqian").isEmpty() || getRuleVal("xlbiaotihou").isEmpty() || getRuleVal("xlbiaotihou").equals("空")) && !getRuleVal("是否启用播放线路名").equals("1")){
